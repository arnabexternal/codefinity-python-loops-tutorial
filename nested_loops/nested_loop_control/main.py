...
for travel_cost in travel_costs:
    index = 0
    significant_expense_found = 0
    while index in range(0,len(travel_cost)):
        if travel_cost[index] > 200:
            significant_expense_found += 1
            significant_expenses.append(travel_cost[index])
            break
        index += 1
    if significant_expense_found == 0:
        print(f"significant_expense_found = {significant_expense_found}")
        significant_expenses.append(None)
